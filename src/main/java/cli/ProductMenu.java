...
public class ProductMenu extends BaseMenu {
    ...
    public void listElectronics() {
        try {
            ...
            list.forEach(e ->
                print(
                    String.format(
                        "  [ID:%-2d]  %-30s  %.2f PLN  (stock: %d)",
                        e.id(),
                        e.name(),
                        e.basePrice().doubleValue(),
                        e.quantity()
                    )
                )
            );
    ...
    public void configureComputerAndAdd() {
        ...
        for (int i = 0; i < processors.length; i++) print(
            String.format(
                "  %d. %-30s  +%.0f PLN",
                i + 1,
                processors[i].getDescription(),
                processors[i].getPrice().doubleValue()
            )
        );
        int pIdx = readInt() - 1;
        ...
        print(NL + "  Choose RAM:");
        Ram[] rams = Ram.values();
        for (int i = 0; i < rams.length; i++) print(
            String.format("  %d. %-4dGB  +%.0f PLN", i + 1, rams[i].getCapacity(), rams[i].getPrice().doubleValue())
        );
        int rIdx = readInt() - 1;

        print(NL + "  Choose storage:");
        StorageType[] storages = StorageType.values();
        for (int i = 0; i < storages.length; i++) print(
            String.format(
                "  %d. %-30s  +%.0f PLN",
                i + 1,
                storages[i].getDescription(),
                storages[i].getPrice().doubleValue()
            )
        );
        int sIdx = readInt() - 1;
        ...
        for (int i = 0; i < gpus.length; i++) print(
            String.format("  %d. %-30s  +%.0f PLN", i + 1, gpus[i].getDescription(), gpus[i].getPrice().doubleValue())
        );
        int gIdx = readInt() - 1;
    ...
    public void configureSmartphoneAndAdd() {
        ...
        for (int i = 0; i < colors.length; i++) print(
            String.format(
                "  %d. %-15s  +%.0f PLN",
                i + 1,
                colors[i].getDescription(),
                colors[i].getPrice().doubleValue()
            )
        );
        int cIdx = readInt() - 1;
        ...
        for (int i = 0; i < batteries.length; i++) print(
            String.format(
                "  %d. %-20s  +%.0f PLN",
                i + 1,
                batteries[i].getDescription(),
                batteries[i].getPrice().doubleValue()
            )
        );
        int bIdx = readInt() - 1;
        ...
        for (int i = 0; i < accessories.length; i++) print(
            String.format(
                "  %d. %-20s  +%.0f PLN",
                i + 1,
                accessories[i].getDescription(),
                accessories[i].getPrice().doubleValue()
            )
        );
        ...
        try {
            ...
            productFacade.updateSmartphone(
                productId,
