...
public class ComputerService {
    ...
    public ComputerDto create(CreateComputerRequest request) {
        ...
        Computer computer = new Computer(null, request.name(), request.basePrice(), request.quantity(), configuration);
    ...
    public ComputerDto update(Long id, UpdateComputerRequest request) {
        ...
        computer
            ...
            .configure(request.processor(), request.ram(), request.storageType(), request.graphicsCard());
    ...
    public void delete(Long id) {
        computerRepository.findById(id).orElseThrow(() -> new ProductNotFoundException(ProductType.COMPUTER, id));

        computerRepository.delete(id);
    }

    public List<ComputerDto> getAll() {
        return computerRepository.getAll().stream().map(ComputerMapper::toDTO).toList();
    }
}
