...
class CartIT {
    ...
    void addElectronics_cartContainsItemWithCorrectPriceAndQuantity() {
        ...
        CartDto cart = cartService.addProduct(customer.id(), monitor.id(), ProductType.ELECTRONICS, 3);
    ...
    void addComputer_cartReflectsConfigurationPrice() {
        ...
        ComputerDto computer = computerService.create(
        ...
        CartDto cart = cartService.addProduct(customer.id(), computer.id(), ProductType.COMPUTER, 1);
    ...
    void addSmartphone_cartReflectsColorBatteryAndAccessoryPrices() {
        ...
        SmartphoneDto phone = smartphoneService.create(
        ...
        CartDto cart = cartService.addProduct(customer.id(), phone.id(), ProductType.SMARTPHONE, 2);
    ...
    void addSameProductTwice_quantityAccumulated_notDuplicated() {
        ...
        CartDto cart = cartService.addProduct(customer.id(), keyboard.id(), ProductType.ELECTRONICS, 3);
    ...
    void addMultipleDifferentProducts_totalIsSumOfAll() {
        ...
        CartDto cart = cartService.addProduct(customer.id(), mouse.id(), ProductType.ELECTRONICS, 4);
    ...
    void addProduct_exceedingStock_throws_cartUnchanged() {
        ElectronicsDto item = electronicsService.create(new CreateElectronicsRequest("SSD", new BigDecimal("400"), 3));
        ...
        assertThatExceptionOfType(InsufficientStockException.class).isThrownBy(() ->
            cartService.addProduct(customer.id(), item.id(), ProductType.ELECTRONICS, 5)
        );
    ...
    void addProduct_variousOverstockScenarios_alwaysThrows(int stock, int requested) {
        ...
        assertThatExceptionOfType(InsufficientStockException.class).isThrownBy(() ->
            cartService.addProduct(customer.id(), item.id(), ProductType.ELECTRONICS, requested)
        );
    }
    ...
    void addProduct_unknownProductId_throws() {
        ...
        assertThatExceptionOfType(ProductNotFoundException.class).isThrownBy(() ->
            cartService.addProduct(customer.id(), 999L, ProductType.ELECTRONICS, 1)
        );
    }
    ...
    void removeProduct_cartUpdatedCorrectly() {
        ...
        cartService.addProduct(customer.id(), keyboard.id(), ProductType.ELECTRONICS, 2);
        CartDto after = cartService.removeProduct(customer.id(), monitor.id(), ProductType.ELECTRONICS);
    ...
    void clearCart_removesAllItems_totalIsZero() {
        ElectronicsDto a = electronicsService.create(new CreateElectronicsRequest("A", new BigDecimal("100"), 5));
        ElectronicsDto b = electronicsService.create(new CreateElectronicsRequest("B", new BigDecimal("200"), 5));
        CustomerDto customer = newCustomer("User", "user@example.com");
