...
class CustomerAuthIT {
    ...
    void register_duplicateEmailThrows_secondCustomerNotPersisted() {
        customerService.createCustomer(new CreateCustomerRequest("First", "shared@example.com", "Password123"));

        assertThatExceptionOfType(EmailAlreadyInUseException.class)
            .isThrownBy(() ->
                customerService.createCustomer(new CreateCustomerRequest("Second", "shared@example.com", "Other123"))
            )
    ...
    void register_duplicateEmailCaseInsensitive() {
        customerService.createCustomer(new CreateCustomerRequest("First", "user@example.com", "Password123"));

        assertThatExceptionOfType(EmailAlreadyInUseException.class).isThrownBy(() ->
            customerService.createCustomer(new CreateCustomerRequest("Second", "USER@EXAMPLE.COM", "Other123"))
        );
    }
    ...
    void register_invalidEmailFormatThrows(String badEmail) {
        assertThatExceptionOfType(InvalidProductException.class).isThrownBy(() ->
            customerService.createCustomer(new CreateCustomerRequest("Test", badEmail, "Password123"))
        );
    }
    ...
    void register_passwordTooShortThrows(String shortPwd) {
        assertThatExceptionOfType(InvalidProductException.class).isThrownBy(() ->
            customerService.createCustomer(new CreateCustomerRequest("Test", "test@example.com", shortPwd))
        );
    }
    ...
    void register_blankNameThrows() {
        assertThatExceptionOfType(InvalidProductException.class).isThrownBy(() ->
            customerService.createCustomer(new CreateCustomerRequest("   ", "test@example.com", "Password123"))
        );
    }
    ...
    void register_multipleValidationErrorsAggregated() {
        assertThatExceptionOfType(InvalidProductException.class)
            .isThrownBy(() -> customerService.createCustomer(new CreateCustomerRequest("", "bad", "")))
    ...
    void login_correctCredentials_returnsCustomerDto() {
        ...
        CustomerDto loggedIn = customerService.login(new LoginRequest("piotr@example.com", "Secure456"));
    ...
    void login_emailCaseInsensitive() {
        customerService.createCustomer(new CreateCustomerRequest("User", "user@example.com", "Password123"));

        CustomerDto loggedIn = customerService.login(new LoginRequest("USER@EXAMPLE.COM", "Password123"));
    ...
    void login_wrongPassword_sameExceptionAsUnknownEmail_noEnumeration() {
        ...
        assertThatExceptionOfType(InvalidCredentialsException.class).isThrownBy(() ->
            customerService.login(new LoginRequest("user@example.com", "WrongPassword"))
        );

        assertThatExceptionOfType(InvalidCredentialsException.class).isThrownBy(() ->
            customerService.login(new LoginRequest("ghost@example.com", "Password123"))
        );
    }
    ...
    void updateCustomer_nameAndEmailChanged() {
        ...
        CustomerDto updated = customerService.updateCustomer(
            original.id(),
    ...
    void updateCustomer_emailTakenByAnotherCustomer_throws() {
        customerService.createCustomer(new CreateCustomerRequest("Alice", "alice@example.com", "Password123"));
        CustomerDto bob = customerService.createCustomer(
            new CreateCustomerRequest("Bob", "bob@example.com", "Password123")
        );

        assertThatExceptionOfType(EmailAlreadyInUseException.class).isThrownBy(() ->
            customerService.updateCustomer(bob.id(), new UpdateCustomerRequest("Bob", "alice@example.com"))
        );
    }
    ...
    void updateCustomer_nonExistentId_throws() {
        assertThatExceptionOfType(CustomerNotFoundException.class).isThrownBy(() ->
            customerService.updateCustomer(999L, new UpdateCustomerRequest("X", "x@example.com"))
        );
    }
    ...
    void changePassword_correctCurrentPassword_succeeds() {
        ...
        customerService.changePassword(customer.id(), new ChangePasswordRequest("OldPass123", "NewPass456"));

        assertThatExceptionOfType(InvalidCredentialsException.class).isThrownBy(() ->
            customerService.login(new LoginRequest("user@example.com", "OldPass123"))
        );

        CustomerDto loggedIn = customerService.login(new LoginRequest("user@example.com", "NewPass456"));
    ...
    void changePassword_wrongCurrentPassword_throws_passwordUnchanged() {
        ...
        assertThatExceptionOfType(InvalidCredentialsException.class).isThrownBy(() ->
            customerService.changePassword(customer.id(), new ChangePasswordRequest("WrongPass", "NewPass456"))
        );

        assertThatNoException()
            .isThrownBy(() -> customerService.login(new LoginRequest("user@example.com", "OldPass123")));
    }
    ...
    void getAllCustomers_returnsAll() {
        ...
        assertThat(all).extracting(CustomerDto::name).containsExactlyInAnyOrder("A", "B", "C");
    }
