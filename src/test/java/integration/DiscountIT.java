...
class DiscountIT {
    ...
    private CreateDiscountRequest percentage(String code, BigDecimal value, BigDecimal minOrder) {
        return CreateDiscountRequest.of(code, "desc", DiscountType.PERCENTAGE,
                value, minOrder, now.minusDays(1), now.plusDays(30));
    }

    private CreateDiscountRequest fixed(String code, BigDecimal value, BigDecimal minOrder) {
        return CreateDiscountRequest.of(code, "desc", DiscountType.FIXED_AMOUNT,
                value, minOrder, now.minusDays(1), now.plusDays(30));
    }
    ...
    void createDiscount_persistsAllFields() {
        DiscountDto dto = discountService.createDiscount(
                CreateDiscountRequest.of("WELCOME10", "Welcome deal", DiscountType.PERCENTAGE,
                        new BigDecimal("10"), BigDecimal.ZERO,
                        now.minusDays(1), now.plusYears(1)));
    ...
    void createDiscount_duplicateCodeThrows_firstOneUnchanged() {
        ...
        assertThatExceptionOfType(InvalidProductException.class)
                .isThrownBy(() -> discountService.createDiscount(
                        percentage("SALE10", new BigDecimal("20"), BigDecimal.ZERO)))
    ...
    void createDiscount_validToBeforeValidFrom_throws() {
        assertThatExceptionOfType(InvalidProductException.class)
                .isThrownBy(() -> discountService.createDiscount(
                        CreateDiscountRequest.of("BAD", "bad", DiscountType.PERCENTAGE,
                                new BigDecimal("10"), BigDecimal.ZERO,
                                now.plusDays(5), now.minusDays(1))));
    }
    ...
    void createDiscount_blankCodeThrows(String code) {
        assertThatExceptionOfType(InvalidProductException.class)
                .isThrownBy(() -> discountService.createDiscount(
                        CreateDiscountRequest.of(code, "desc", DiscountType.PERCENTAGE,
                                new BigDecimal("10"), BigDecimal.ZERO,
    ...
    void createDiscount_negativeValueThrows() {
        assertThatExceptionOfType(InvalidProductException.class)
                .isThrownBy(() -> discountService.createDiscount(
                        percentage("NEG", new BigDecimal("-5"), BigDecimal.ZERO)));
    }
    ...
    void applyDiscount_fixedAmount_correctResultWithFloor(
            BigDecimal amount, BigDecimal base, BigDecimal expected) {
    ...
    void applyDiscount_expiredCode_throws() {
        discountService.createDiscount(CreateDiscountRequest.of(
                "EXPIRED", "old", DiscountType.PERCENTAGE,
                new BigDecimal("10"), BigDecimal.ZERO,
                now.minusDays(30), now.minusDays(1)));

        assertThatExceptionOfType(DiscountNotFoundException.class)
                .isThrownBy(() -> discountService.applyDiscount("EXPIRED", new BigDecimal("1000")));
    }

    @Test
    void applyDiscount_futureCode_throws() {
        discountService.createDiscount(CreateDiscountRequest.of(
                "FUTURE", "not yet", DiscountType.PERCENTAGE,
                new BigDecimal("10"), BigDecimal.ZERO,
                now.plusDays(5), now.plusDays(30)));

        assertThatExceptionOfType(DiscountNotFoundException.class)
                .isThrownBy(() -> discountService.applyDiscount("FUTURE", new BigDecimal("1000")));
    }

    @Test
    void deactivate_discountNoLongerApplicableOrVisibleAsActive() {
        DiscountDto dto = discountService.createDiscount(
                percentage("ACTIVE", new BigDecimal("10"), BigDecimal.ZERO));
        ...
        assertThatExceptionOfType(DiscountNotFoundException.class)
                .isThrownBy(() -> discountService.applyDiscount("ACTIVE", new BigDecimal("1000")));
    }
    ...
    void getAllActive_returnsOnlyValidDiscounts() {
        ...
        discountService.createDiscount(CreateDiscountRequest.of(
                "EXPIRED2", "old", DiscountType.PERCENTAGE, new BigDecimal("15"),
                BigDecimal.ZERO, now.minusDays(10), now.minusDays(1)));

        List<DiscountDto> active = discountService.getAllActive();

        assertThat(active).hasSize(2);
        assertThat(active).extracting(DiscountDto::code)
    ...
    void getByCode_unknownCode_throws() {
        assertThatExceptionOfType(DiscountNotFoundException.class)
                .isThrownBy(() -> discountService.getByCode("GHOST"));
    }
