...
class ProductCatalogIT {
    ...
    void createComputer_persistsAndReturnsWithConfigurationPrice() {
        ComputerDto dto = computerService.create(new CreateComputerRequest(
                "Dell XPS 15", new BigDecimal("4000"), 10,
    ...
    void createComputer_configurationPriceMatchesEnumValues(
            Processor proc, Ram ram, StorageType storage,
            GraphicsCard gpu, BigDecimal expectedCfgPrice) {

        ComputerDto dto = computerService.create(new CreateComputerRequest(
                "PC", new BigDecimal("1000"), 1, proc, ram, storage, gpu));

        assertThat(dto.totalPrice().subtract(dto.basePrice()))
                .isEqualByComparingTo(expectedCfgPrice);
    }

    @Test
    void updateComputer_allFieldsReflectedImmediately() {
        ComputerDto created = computerService.create(new CreateComputerRequest(
                "Old PC", new BigDecimal("2000"), 3,
                Processor.INTEL_I5, Ram.RAM_8GB, StorageType.SSD_512GB, GraphicsCard.INTEGRATED));

        ComputerDto updated = computerService.update(created.id(), new UpdateComputerRequest(
                "New PC", new BigDecimal("2500"), 5,
    ...
    void deleteComputer_removedFromRepository() {
        ...
        assertThatExceptionOfType(ProductNotFoundException.class)
                .isThrownBy(() -> computerService.getById(dto.id()));
    }

    @Test
    void getAllComputers_returnsAllCreatedInOrder() {
        computerService.create(new CreateComputerRequest("A", new BigDecimal("1000"), 1,
                Processor.INTEL_I5, Ram.RAM_8GB, StorageType.SSD_512GB, GraphicsCard.INTEGRATED));
        computerService.create(new CreateComputerRequest("B", new BigDecimal("2000"), 1,
                Processor.INTEL_I7, Ram.RAM_16GB, StorageType.SSD_1TB, GraphicsCard.RTX_3050));
        computerService.create(new CreateComputerRequest("C", new BigDecimal("3000"), 1,
                Processor.INTEL_I9, Ram.RAM_32GB, StorageType.SSD_2TB, GraphicsCard.RTX_4070));

        List<ComputerDto> all = computerService.getAll();

        assertThat(all).hasSize(3);
        assertThat(all).extracting(ComputerDto::name)
    ...
    void getComputerById_throwsWhenNotFound() {
        assertThatExceptionOfType(ProductNotFoundException.class)
                .isThrownBy(() -> computerService.getById(999L));
    }

    @Test
    void createSmartphone_persistsAndReturnsWithConfigurationPrice() {
        SmartphoneDto dto = smartphoneService.create(new CreateSmartphoneRequest(
                "iPhone 15 Pro", new BigDecimal("5000"), 20,
                Set.of(Accessory.CHARGER, Accessory.PHONE_CASE),
                BatteryCapacity.BATTERY_5000, Color.GOLD));
        ...
        assertThat(dto.smartphoneConfiguration().accessories())
                .containsExactlyInAnyOrder(Accessory.CHARGER, Accessory.PHONE_CASE);
    ...
    void createSmartphone_noAccessories_onlyColorAndBatteryAdded() {
        SmartphoneDto dto = smartphoneService.create(new CreateSmartphoneRequest(
                "Basic Phone", new BigDecimal("2000"), 5,
                Set.of(), BatteryCapacity.BATTERY_4000, Color.BLACK));
    ...
    void updateSmartphone_configurationAndPriceUpdated() {
        SmartphoneDto created = smartphoneService.create(new CreateSmartphoneRequest(
                "Phone", new BigDecimal("3000"), 10,
                Set.of(), BatteryCapacity.BATTERY_3000, Color.BLACK));

        SmartphoneDto updated = smartphoneService.update(created.id(), new UpdateSmartphoneRequest(
                "Phone Pro", new BigDecimal("3500"), 8,
        ...
        assertThat(updated.smartphoneConfiguration().accessories())
                .containsExactlyInAnyOrder(Accessory.CHARGER, Accessory.HEADPHONES);
    ...
    void deleteSmartphone_removedFromRepository() {
        SmartphoneDto dto = smartphoneService.create(new CreateSmartphoneRequest(
                "Temp Phone", new BigDecimal("2000"), 1,
                Set.of(), BatteryCapacity.BATTERY_4000, Color.BLACK));
        ...
        assertThatExceptionOfType(ProductNotFoundException.class)
                .isThrownBy(() -> smartphoneService.getById(dto.id()));
    }
    ...
    void updateElectronics_allFieldsReflected() {
        ...
        ElectronicsDto updated = electronicsService.update(created.id(),
    ...
    void deleteElectronics_throwsOnSubsequentFetch() {
        ElectronicsDto dto = electronicsService.create(
                new CreateElectronicsRequest("Temp", new BigDecimal("500"), 1));

        electronicsService.delete(dto.id());

        assertThat(electronicsService.getAll()).isEmpty();
        assertThatExceptionOfType(ProductNotFoundException.class)
                .isThrownBy(() -> electronicsService.getById(dto.id()));
    }

    @Test
    void allThreeProductTypes_idsAreIndependent() {
        ComputerDto c = computerService.create(new CreateComputerRequest(
                "PC", new BigDecimal("1000"), 1,
                Processor.INTEL_I5, Ram.RAM_8GB, StorageType.SSD_512GB, GraphicsCard.INTEGRATED));
        SmartphoneDto s = smartphoneService.create(new CreateSmartphoneRequest(
                "Phone", new BigDecimal("2000"), 1,
                Set.of(), BatteryCapacity.BATTERY_4000, Color.BLACK));
        ElectronicsDto e = electronicsService.create(
                new CreateElectronicsRequest("Monitor", new BigDecimal("500"), 1));

        assertThat(List.of(c.id(), s.id(), e.id()))
                .allMatch(id -> id >= 1L);
    }
    ...
    void createElectronics_throwsForBlankName(String name) {
        assertThatExceptionOfType(InvalidProductException.class)
                .isThrownBy(() -> electronicsService.create(
                        new CreateElectronicsRequest(name, new BigDecimal("100"), 1)));
    }
    ...
    void createElectronics_throwsForNegativePrice() {
        assertThatExceptionOfType(InvalidProductException.class)
                .isThrownBy(() -> electronicsService.create(
                        new CreateElectronicsRequest("Monitor", new BigDecimal("-1"), 1)));
    }
}