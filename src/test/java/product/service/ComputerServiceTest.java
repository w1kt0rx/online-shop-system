...
class ComputerServiceTest {
    ...
    void shouldCreateComputerAndSaveToRepository() {
        ...
        ComputerDto result = computerService.create(
    ...
    void shouldCreateComputerForVariousConfigurations(String name, BigDecimal price, Processor proc, Ram ram) {
        when(computerRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));

        ComputerDto result = computerService.create(
            new CreateComputerRequest(name, price, 1, proc, ram, StorageType.SSD_512GB, GraphicsCard.INTEGRATED)
        );
    ...
    void shouldThrowWhenDeletingNonExistentComputer() {
        ...
        assertThatExceptionOfType(ProductNotFoundException.class).isThrownBy(() -> computerService.delete(99L));
    ...
    void shouldUpdateComputerFields() {
        ...
        ComputerDto result = computerService.update(
            1L,
